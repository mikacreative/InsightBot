...
def validate_task_definition(task_id: str, task_def: dict[str, Any], channels_data: dict[str, Any]) -> dict[str, Any]:
    raw_legacy_search = (task_def or {}).get("search", {}) or {}
    ...
    if (
        isinstance(raw_legacy_search, dict)
        and raw_legacy_search.get("enabled")
        and not search.get("enabled")
    ):
        search = raw_legacy_search
